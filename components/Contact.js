...
export default function Contact() {
  ...
  const inputStyle = { width:'100%', background:'#080a0f', border:'1px solid #1e2a3a', borderRadius:'10px', padding:'14px 16px', fontSize:'14px', color:'#94a3b8', outline:'none', fontFamily:'inherit', transition:'border-color 0.2s' }
  const socialLinks = [
  ...
  return (
    <section id="contact" style={{padding:'100px 0',borderTop:'1px solid #0f1520',background:'#080a0f'}}>
      <div style={{maxWidth:'1280px',margin:'0 auto',padding:'0 24px'}}>
        <p style={{fontSize:'12px',fontWeight:600,color:'#64748b',textTransform:'uppercase',letterSpacing:'0.1em',marginBottom:'48px'}}>Contact</p>
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'24px'}}>
          <div style={{background:'#0d1117',border:'1px solid #0f1520',borderRadius:'20px',padding:'40px'}}>
            <h3 style={{fontSize:'24px',fontWeight:600,color:'#f1f5f9',marginBottom:'12px'}}>Let's connect!</h3>
            <p style={{fontSize:'15px',color:'#64748b',lineHeight:1.7,marginBottom:'36px'}}>Open to QA Engineer roles, full-stack developer positions, and interesting collaborations. I usually respond within 24 hours.</p>
            <div style={{display:'flex',flexDirection:'column',gap:'12px'}}>
              {socialLinks.map(({icon,label,sub,href}) => (
                <a key={label} href={href} target={label!=='Phone'&&label!=='Email'?'_blank':undefined} rel="noreferrer"
                  style={{display:'flex',alignItems:'center',gap:'16px',background:'#0a0e14',border:'1px solid #1e2a3a',borderRadius:'12px',padding:'16px 20px',textDecoration:'none'}}>
          ...
          <div style={{background:'#0d1117',border:'1px solid #0f1520',borderRadius:'20px',padding:'40px'}}>
            <form onSubmit={handleSubmit} style={{display:'flex',flexDirection:'column',gap:'20px'}}>
              ...
              <div>
                ...
                <textarea name="message" value={form.message} onChange={handleChange} required rows={6} placeholder="Tell me about your opportunity or project..." style={{...inputStyle,resize:'none'}} />
              </div>
              <button type="submit" disabled={status==='sending'} style={{background:'#2563eb',color:'#fff',fontSize:'15px',padding:'16px',borderRadius:'10px',border:'none',cursor:'pointer',fontWeight:500,fontFamily:'inherit',opacity:status==='sending'?0.6:1}}>
                {status==='sending' ? 'Sending...' : 'Send Message'}
              </button>
              {status==='success' && <p style={{fontSize:'13px',color:'#22c55e',textAlign:'center'}}>Message sent! I will get back to you soon.</p>}
              {status==='error' && <p style={{fontSize:'13px',color:'#f87171',textAlign:'center'}}>Something went wrong. Please email me directly.</p>}
            </form>
          </div>
        </div>
      </div>
